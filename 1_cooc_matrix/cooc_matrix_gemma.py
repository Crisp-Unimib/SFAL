...
@torch.no_grad()
def main(args):
    # DEBUG set all dict values to 1
    thresholds_dict = {k: 1 for k in thresholds_dict.keys()}
    ...
    print("Lodaded SAEs:")
    for sae_name in res_saes_names:
        print(f"\t{sae_name}")

    try:
        threshold_values = [thresholds_dict[str(i)] for i in range(params['W_enc'].shape[1])]
        threshold_tensor = torch.tensor(threshold_values, device=device, dtype=dtype)

    except KeyError as e:
         raise ValueError(f"Threshold dictionary missing key: {e}. Ensure it has entries '0' to '{params['W_enc'].shape[1]-1}'.")

    except Exception as e:
         raise ValueError(f"Error creating threshold tensor: {e}")
    ...
    for i, doc in tqdm(enumerate(dataset), total=args.n_docs):
        ...
        for layeri, sae_name, sae in zip(args.layers, res_saes_names, res_saes):
            ...
            sae_acts = (sae_acts > threshold_tensor).float()
